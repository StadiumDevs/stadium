/**
 * Remove duplicate projects across ALL hackathons, keeping the one with the most data.
 *
 * Usage (from server/):
 *   SUPABASE_URL=... SUPABASE_SERVICE_ROLE_KEY=... node scripts/remove-all-duplicates.js
 */
...
async function main() {
  ...
  const { data: projects, error: projError } = await supabase
    .from('projects')
    .select('id, project_name, hackathon_id, created_at, updated_at, m2_status, completion_date')
  ...
  // Group by project name + hackathon (case-insensitive)
  const groups = new Map();
  for (const p of projects || []) {
    const key = `${p.project_name.toLowerCase().trim()}-${p.hackathon_id}`;
    if (!groups.has(key)) {
      groups.set(key, []);
    }
  ...
  for (const [key, duplicates] of groups.entries()) {
    ...
    const [name, hackathon] = key.split('-', 2);
    console.log(`\n${duplicates[0].project_name} (${hackathon}): ${duplicates.length} entries`);
  ...
  console.log(`\nDone. Kept: ${kept} unique projects, deleted: ${deleted} duplicates`);
  console.log(`Total projects before: ${projects.length}, after: ${projects.length - deleted}`);
}
